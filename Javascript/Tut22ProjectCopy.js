...
<div class="alert alert-danger" role="alert">
    Alert! &nbsp Add something to note  </div>`
    setTimeout(() => {
      alert.innerHTML=``
    }, 3000);
...
<div class="alert alert-success" role="alert">
  ...
  setTimeout(() => {
    ...
    // For first time if title or textArea is empty pass empty arrry to localStorage
  
  }
  // for blank note remove from array
  else if (addTxt.value.trim().length == 0) {
    notesObj = JSON.parse(notes);
    let myObj = {
      title: addTitle.value,
      note: addTxt.value,
...
function showNotes() {
  ...
  if (notesObj.length != 0) {
    ...
    noteTxt.innerHTML = `Nothing to show! use "Add note" to add a note`;
  }
}
...
function editNote(editId) {
  ...
  console.log(editTxt);
  // let noTextAreas = document.getElementsByClassName(".editDiv textArea").length;

  // console.log(noTextAreas);

  editTxt.innerHTML = `<textarea id="textArea1" class="my-3">${notesObj[editId - 1].title}</textarea>
      <textarea class="textArea" id="textArea" rows="4" placeholder="click here to edit">${notesObj[editId - 1].note}</textarea>
      <div class="card-body">
      <button id="${editId - 1}" onclick="deleteNote(this.id)" class="btn row btn-primary">Delete Note</button>
